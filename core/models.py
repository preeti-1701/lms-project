...
class UserSession(models.Model):
    """Tracks active user sessions for single-session enforcement (SRS 3.1)."""
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='sessions')
    token_key = models.CharField(max_length=40, unique=True, help_text='DRF token key')
    ip_address = models.GenericIPAddressField(null=True, blank=True)
    user_agent = models.TextField(blank=True, help_text='Browser/device info')
    device_label = models.CharField(max_length=100, blank=True, help_text='Friendly device name')
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    def __str__(self):
        ...
        return f"{status} {self.user.username} on {self.device_label or 'unknown'} ({self.ip_address})"
