...
export default function Lesson({ user }) {
  ...
  useEffect(() => {
    ...
    const handleContextMenu = (e) => {
      e.preventDefault()
      showNotice('Right-click is disabled to protect course content.')
    }
    const handleKeyDown = (e) => {
      if (
        ...
        e.preventDefault()
        showNotice('This action is blocked on protected content.')
      }
    }
    const handleKeyUp = (e) => {
      if (e.key === 'PrintScreen') {
        navigator.clipboard.writeText('').catch(() => {})
        showNotice('Screenshot detected. This has been logged.')
      }
    }
        ...
        {lesson.video_url && (
          <div className="video-link-card">
            ...
            <div className="video-link-notice">
              🔒 This link is shared with <strong>{user.username}</strong>. Unauthorized sharing is prohibited.
            </div>
          </div>
        )}
