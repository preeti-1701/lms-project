          ...
          <div className="course-grid-rich">
            {completedCourses.map(e => (
              ...
              >
                ...
                <div className="course-card-top" onClick={() => navigate(`/courses/${e.course.slug}`)} style={{ cursor: 'pointer' }}>
                  <div className="course-emoji-big">{e.course.thumbnail_emoji}</div>
                  <div className={`level-pill level-${e.course.level}`}>{e.course.level}</div>
                </div>
                <div className="course-category">{e.course.category}</div>
                <h3 className="course-title-rich" onClick={() => navigate(`/courses/${e.course.slug}`)} style={{ cursor: 'pointer' }}>{e.course.title}</h3>
                <div className="course-instr-row">
                  <div className="instr-dot"></div>
                  <span>{e.course.instructor_name}</span>
                </div>
                <div style={{ paddingTop: '1rem', borderTop: '1px solid var(--rule)', marginTop: '1rem' }}>
                  <a
                    href={`http://127.0.0.1:8000/api/certificate/${e.course.slug}/`}
                    onClick={async (event) => {
                      event.preventDefault()
                      event.stopPropagation()
                      // Download with auth header — fetch then save
                      try {
                        const token = localStorage.getItem('coursify_token')
                        const res = await fetch(`http://127.0.0.1:8000/api/certificate/${e.course.slug}/`, {
                        ...
                        a.href = url
                        a.download = `Coursify-${e.course.slug}-certificate.pdf`
        ...
        <div className="dash-section">
          <div className="empty-state-big">
            ...
            <button
              ...
              style={{ marginTop: '1.5rem', padding: '0.9rem 1.75rem' }}
            >
              Browse catalog →
            </button>
          </div>
